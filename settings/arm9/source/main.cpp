...
void opt_update()
{
	...
	while (1) {
		...
		// if Power LED on DSi is set to purple
...
//#pragma endregion
	...
	if ((sdFound() && !sys().arm7SCFGLocked()) || (sys().isRegularDS() && (io_dldi_data->ioInterface.features & FEATURE_SLOT_GBA))) {
		bootstrapPage
